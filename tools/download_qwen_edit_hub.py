...
class QwenEditDownloader:
        ...
        try:
            ...
            if cache_path.exists():
                for file_info in repo_data.siblings:
                    ...
                    if (
                        local_file.exists()
                        and local_file.stat().st_size == file_info.size
                    ):
                ...
                if status["files_downloaded"]:
                    ...
                    print(
                        f"💾 Downloaded: {self._format_size(status['downloaded_size'])}"
                    )
    ...
    def download_with_progress(self, resume: bool = True) -> bool:
        ...
        if resume and status["files_downloaded"]:
            print(
                f"🔄 Resuming download ({len(status['missing_files'])} files remaining)"
            )
    ...
    def download_selective_files(self, file_patterns: List[str]) -> bool:
        ...
        try:
            ...
            for pattern in file_patterns:
                matching_files = [
                    f for f in repo_data.siblings if pattern in f.rfilename
                ]
...
def main():
    ...
    parser.add_argument(
        "--cache-dir", default="./models/qwen-image-edit", help="Custom cache directory"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    parser.add_argument("--no-resume", action="store_true", help="Start fresh download")
    parser.add_argument(
        "--status-only", action="store_true", help="Only check download status"
    )
    parser.add_argument(
        "--cleanup", action="store_true", help="Clean up partial downloads"
    )
    parser.add_argument(
        "--selective", nargs="+", help="Download only files matching patterns"
    )
