...
def check_pytorch_configuration() -> Dict[str, any]:
    ...
    if torch.cuda.is_available():
        ...
        for var, description in perf_vars.items():
            ...
            elif var == 'OMP_NUM_THREADS' and (value == 'Not set' or int(value) if value.isdigit() else 0 < 16):
...
def diagnose_generation_bottlenecks() -> List[str]:
    ...
    if torch.cuda.is_available():
        ...
        issues.append("CPU offloading may be enabled - keeps model components on slow CPU")
        ...
        if os.path.exists(config_file):
            with open(config_file, 'r') as f:
                ...
                if 'max_memory' in content and '"12GB"' in content:
                    issues.append(f"Conservative VRAM limit in {config_file} - only using 12GB of 16GB")
                
                if 'enable_sequential_cpu_offload": True' in content:
                    issues.append(f"Sequential CPU offload enabled in {config_file} - major performance killer")
                
                if 'low_cpu_mem_usage": True' in content:
                    issues.append(f"Low CPU memory usage enabled in {config_file} - unnecessary with 128GB RAM")
    
    # 3. Check attention slicing
    print("  • Attention Slicing: Analyzing...")
    issues.append("Attention slicing may be enabled - trades speed for memory on high-VRAM systems")
...
def run_performance_benchmark() -> float:
    ...
    with torch.no_grad():
        ...
        latents = torch.randn(batch_size, latent_channels, height//8, width//8, device=device, dtype=dtype)
        ...
        for step in range(10):
            ...
            noise_pred = torch.nn.functional.conv2d(
                latents, 
                torch.randn(latent_channels, latent_channels, 3, 3, device=device, dtype=dtype),
                padding=1
            )
...
def generate_optimization_recommendations(system_info: Dict, issues: List[str]) -> List[str]:
    ...
    recommendations.append("  → Use HighEndQwenImageGenerator instead of QwenImageGenerator")
