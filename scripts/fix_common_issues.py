...
def fix_linting_issues():
    ...
    # Ruff auto-fix
    success, _ = run_command(
        ["ruff", "check", "--fix", "src/", "tests/", "examples/", "scripts/", "launch.py"],
        "Auto-fixing with Ruff"
    )
    
    # autopep8 for additional PEP8 fixes
    success, _ = run_command(
        ["autopep8", "--in-place", "--recursive", "--aggressive", "src/", "tests/", "examples/", "scripts/", "launch.py"],
...
def fix_line_endings():
    ...
    python_files = []
    for pattern in ["src/**/*.py", "tests/**/*.py", "examples/**/*.py", "scripts/**/*.py", "*.py"]:
...
def main():
    ...
    parser.add_argument("--install-tools", action="store_true", help="Install missing development tools")
    parser.add_argument("--format-only", action="store_true", help="Only run formatting fixes")
    parser.add_argument("--lint-only", action="store_true", help="Only run linting fixes")
    ...
    print("\n" + "=" * 50)
    print(f"🏁 Fix Summary: {success_count}/{total_count} operations completed successfully")
