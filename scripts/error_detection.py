...
class ErrorDetector:
    ...
    def add_error(self, category: str, message: str, file_path: str = "", line: int = 0):
    ...
    def add_warning(self, category: str, message: str, file_path: str = "", line: int = 0):
    ...
    def check_python_syntax(self) -> None:
        ...
        python_files = [f for f in python_files if not any(skip in str(f) for skip in ['venv', '__pycache__', '.git', 'build', 'dist'])]
        ...
        for py_file in python_files:
            try:
                ...
                self.add_error("syntax", f"Syntax error: {e.msg}", str(py_file), e.lineno or 0)
    ...
    def check_hardware_requirements(self) -> None:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                self.add_warning("hardware", "⚠️ CUDA not available - will use CPU (slow)")
                
        except ImportError:
            self.add_error("hardware", "❌ PyTorch not available - cannot check hardware")
    ...
    def check_dependencies(self) -> None:
        ...
        try:
            ...
            if missing_packages:
                self.add_warning("dependencies", f"⚠️ Missing packages: {', '.join(missing_packages)}")
            else:
                self.add_info("dependencies", "✅ All core dependencies appear to be installed")
    ...
    def check_model_files(self) -> None:
        ...
        if large_files:
            for file_path, size_mb in large_files:
                self.add_warning("models", f"Large model file detected: {file_path} ({size_mb:.1f}MB)")
        ...
        if models_dir.exists():
            ...
            self.add_info("models", "ℹ️ Models directory not found (will be created on first run)")
    ...
    def generate_report(self, results: Dict[str, Any], output_file: str = "error_detection_report.json") -> None:
        ...
        if results["errors"]:
            ...
            for error in results["errors"]:
                file_info = f" ({error['file']}:{error['line']})" if error['file'] else ""
        ...
        if results["warnings"]:
            ...
            for warning in results["warnings"]:
                file_info = f" ({warning['file']}:{warning['line']})" if warning['file'] else ""
