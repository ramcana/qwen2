...
class IntegrationTestSuite:
    ...
    def run_all_tests(self):
        ...
        for test_name, test_func in tests:
            ...
            try:
                ...
                self.test_results[test_name] = {"success": result, "duration": duration}
    ...
    def test_api_integration(self) -> bool:
        ...
        for method, endpoint, description in endpoints:
            try:
                ...
                print(
                    f"   {description}: {'✅' if success else '❌'} ({response.status_code})"
                )
    ...
    def test_memory_optimization(self) -> bool:
        ...
        try:
            ...
            if memory_info:
                print(
                    f"   📊 VRAM: {memory_info['allocated_gb']}GB / {memory_info['total_gb']}GB"
                )
    ...
    def test_performance(self) -> bool:
        ...
        try:
            ...
            response = requests.post(
                self.api_url + "/generate/text-to-image", json=payload, timeout=60
            )
    ...
    def test_concurrent_requests(self) -> bool:
        ...
        def make_request(request_id):
            ...
            try:
                response = requests.post(
                    self.api_url + "/generate/text-to-image", json=payload, timeout=30
                )
                return {
                    "id": request_id,
                    "success": response.status_code == 200,
        ...
        print(
            f"   📊 Requests: {num_requests}, Successful: {successful}, Queued: {queued}"
        )
            ...
            if response.status_code in [400, 422]:  # Validation errors
                ...
                if response.headers.get("content-type", "").startswith(
                    ...
                    if "detail" in error_data:
                        print(
                            f"   ✅ Error message provided: {error_data['detail'][:50]}..."
                        )
                        return True
                ...
                print(
                    f"   ⚠️ Unexpected response to invalid request: {response.status_code}"
                )
                return False
