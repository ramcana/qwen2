...
def launch_api(config_manager=None, background=True):
    ...
    api_config = (
        ...
        if config_manager
        else {"port": 8000, "host": "0.0.0.0"}
    )

    print(
        f"📡 Starting FastAPI server on http://{api_config['host']}:{api_config['port']}"
    )
...
def launch_gradio(config_manager=None, background=True):
    ...
    gradio_config = (
        config_manager.get_gradio_config() if config_manager else {"port": 7860}
    )
...
def launch_react(config_manager=None, background=True):
    ...
    react_config = (
        config_manager.get_react_config() if config_manager else {"dev_port": 3000}
    )

    print(
        f"⚛️  Starting React development server on http://localhost:{react_config['dev_port']}"
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Qwen-Image Generator Unified Launcher"
    )

    parser.add_argument(
        "--mode",
        choices=["gradio", "react", "both", "api-only"],
        default="both",
    ...
    parser.add_argument(
        "--install-deps", action="store_true", help="Install React dependencies"
    )
    parser.add_argument(
    ...
    try:
        ...
        if background and processes:
            ...
            if not args.no_api:
                api_port = (
                    config_manager.get_api_config()["port"] if config_manager else 8000
                )
            ...
            if args.mode in ["gradio", "both"]:
                gradio_port = (
                    config_manager.get_gradio_config()["port"]
                    if config_manager
                    else 7860
                )
            ...
            if args.mode in ["react", "both"]:
                react_port = (
                    config_manager.get_react_config()["dev_port"]
                    if config_manager
                    else 3000
                )
