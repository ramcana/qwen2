...
def get_memory_optimized_config() -> Dict[str, Any]:
    ...
    print(
        f"🔍 VRAM Status: {available_memory / 1e9:.1f}GB available of {total_memory / 1e9:.1f}GB total"
    )
...
def apply_memory_optimizations(pipeline):
    ...
    try:
        ...
        if MEMORY_OPTIMIZATION_CONFIG["torch_cuda_memory_fraction"] < 1.0:
            ...
            print(
                f"✅ CUDA memory fraction set to {MEMORY_OPTIMIZATION_CONFIG['torch_cuda_memory_fraction']}"
            )
