...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "react": {"enabled": False, "dev_port": 3000, "build_dir": "frontend/build"},
    "api": {"enabled": True, "port": 8000, "host": "0.0.0.0", "auto_start": True},
    "features": {
...
def main():
    ...
    mode_parser.add_argument(
        "mode", choices=["gradio", "react", "both"], help="UI mode to set"
    )
    ...
    feature_parser.add_argument(
        "action", choices=["enable", "disable"], help="Action to perform"
    )

    # Port configuration
    port_parser = subparsers.add_parser("set-port", help="Set port for service")
    port_parser.add_argument(
        "service", choices=["gradio", "react", "api"], help="Service to configure"
    )
    ...
    launch_parser.add_argument(
        "--background", action="store_true", help="Launch in background"
    )
    ...
    if args.command == "status":
        ...
        if service_key in config_manager.config:
            port_key = (
                "port"
                if args.service == "api"
                else ("port" if args.service == "gradio" else "dev_port")
            )
...
def launch_configured_uis(config_manager: UIConfigManager, background: bool = False):
    ...
    if config_manager.is_gradio_enabled():
        ...
        if background:
            process = subprocess.Popen(
                gradio_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
