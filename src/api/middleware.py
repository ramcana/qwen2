...
class MemoryOptimizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if pre_memory and pre_memory.get("usage_percent", 0) > self.memory_threshold * 100:
            logger.warning(f"High memory usage detected: {pre_memory['usage_percent']}%")
        ...
        try:
            ...
            if post_memory:
                response.headers["X-GPU-Memory-Used"] = f"{post_memory['allocated_gb']}GB"
                response.headers["X-GPU-Memory-Percent"] = f"{post_memory['usage_percent']}%"
    ...
    async def _aggressive_cleanup(self):
        ...
        if torch.cuda.is_available():
            ...
            if freed_memory:
                ...
                logger.info(f"Aggressive cleanup completed. Available: {freed_memory['available_gb']}GB")
        ...
        while True:
            try:
                ...
                if memory_info:
                    ...
                    # Log memory status
                    logger.info(f"Memory monitor: {usage_percent}% used ({memory_info['allocated_gb']}GB)")
...
class PerformanceMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "success_rate": round((len(self.request_times) / (len(self.request_times) + self.error_count)) * 100, 2)
        }
