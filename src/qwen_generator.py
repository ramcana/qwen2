...
class QwenImageGenerator:
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                for comp_name in additional_components:
                    if (
                        hasattr(self.pipe, comp_name)
                        and getattr(self.pipe, comp_name) is not None
                    ):
                        try:
    ...
    def _load_qwen_edit_pipeline(self) -> None:
        ...
        if QwenImageEditPipeline is None:
            print(
                "⚠️ QwenImageEditPipeline not available. Install latest diffusers from GitHub."
            )
    ...
    def _verify_edit_pipeline_devices(self) -> bool:
        ...
        try:
            ...
            for comp_name in components:
                if (
                    hasattr(self.edit_pipe, comp_name)
                    and getattr(self.edit_pipe, comp_name) is not None
                ):
                    ...
                    try:
                        ...
                        if comp_device != self.device and comp_device != "unknown":
                            print(
                                f"   🔧 Moving {comp_name} from {comp_device} to {self.device}"
                            )
                            try:
                                component = component.to(self.device)
                        ...
                        print(
                            f"   {comp_name.upper()}: error checking device ({comp_error})"
                        )
                        all_correct = False

            # Check scheduler
            if (
                hasattr(self.edit_pipe, "scheduler")
                and self.edit_pipe.scheduler is not None
            ):
                print(
                    f"   SCHEDULER: present ({type(self.edit_pipe.scheduler).__name__})"
                )
            ...
            for comp_name in components:
                if (
                    ...
                    try:
                        ...
                        print(
                            f"   {comp_name.upper()}: {comp_device} ({param_count:,} params)"
                        )
            ...
            for comp_name in components:
                if (
                    ...
                    try:
                        ...
                        param_count = sum(p.numel() for p in component.parameters())
                        print(
                            f"✅ {comp_name}: moved to {self.device} ({param_count:,} parameters)"
                        )
    ...
    def generate_image(
        ...
        try:
            ...
            num_inference_steps = (
                num_inference_steps or GENERATION_CONFIG["num_inference_steps"]
            )
            ...
            print(
                f"Settings: {width}x{height}, steps: {num_inference_steps}, CFG: {cfg_scale}, seed: {seed}"
            )
            print(
                f"Device: {self.device}, Generator device: {generator.device if hasattr(generator, 'device') else 'cpu'}"
            )
                        ...
                        if component is not None and hasattr(component, "to"):
                            ...
                            try:
                                ...
                                if hasattr(component, "device"):
                                    ...
                                    comp_device = str(
                                        next(component.parameters()).device
                                    )

                                if comp_device != self.device:
                                    print(
                                        f"⚠️ {comp_name} on {comp_device}, expected {self.device}"
                                    )
                                    device_issues = True
                                else:
            ...
            with device_context:
                with torch.no_grad():
                    ...
                    if torch.cuda.is_available():
                        ...
                        print(
                            f"🧹 CUDA synchronized, available memory: {torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_allocated():.1e} bytes"
                        )
    ...
    def generate_img2img(
        ...
        if not self.edit_pipe:
            ...
            error_msg += "\nPossible solutions:\n"
            error_msg += (
                "1. Wait for model download to complete (may take 10-20 minutes)\n"
            )
    ...
    def super_resolution(
        self, image: PIL.Image.Image, scale_factor: int = 2
    ) -> Tuple[Optional[PIL.Image.Image], str]:
